...
for (i in c(1:length(new_loci_head$SNP))){
  ...
  new_gwas_data[(new_gwas_data$SNP %in% snp_list_loc),'locus_start'] = new_gwas_data$POS[index]- 1e6
  new_gwas_data[(new_gwas_data$SNP %in% snp_list_loc),'locus_end'] = new_gwas_data$POS[index]+ 1e6
...
write.table(loci_df, file = loci_info_dir, col.names = TRUE,
            row.names = FALSE, quote = FALSE)
