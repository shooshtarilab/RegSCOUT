...
while [[ "$#" -gt 0 ]]; do
  case $1 in
    ...
    --LD_thr) LD_thr="$2"; shift ;;
    --jaspar_mtx) jaspar_mtx="$2"; shift ;;
    --seurat_obj) seurat_oyj="$2"; shift ;;
    ...
    --gene_ppa_th) gene_ppa_th="$2"; shift;;
...
if [ "$mode" == "ATAC_obj" ]; then
    Rscript EffectSNP.R --jaspar_mtx "$jaspar_mtx" --output_dir "$output_dir" --ci_gwas_dir "$ci_gwas_dir" --genome_build "$genome_build"
    Rscript Peak_cell_extract.R --seurat_obj "$seurat_obj" --output_dir "$output_dir" --peak_th "$peak_th"
    Rscript peak_interaction_extract.R --genome_build "$genome_build" --seurat_obj "$seurat_obj" --output_dir "$output_dir"
