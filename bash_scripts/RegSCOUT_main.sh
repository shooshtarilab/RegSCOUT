...
if [ "$finemap" == "Y" ]; then
    Rscript fgwas_data_prep.R --output_dir "$output_dir" --SNP_ref "$SNP_ref" --Population "$Population" --sum_stats "$sum_stats" --lead_snps "$lead_snps" --plink2_bin "$plink2_bin" --sample_num "$sample_num"
