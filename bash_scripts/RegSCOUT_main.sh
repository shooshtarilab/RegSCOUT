...
if [ "$finemap" == "Y" ]; then
    ...
    Rscript fine_map.R --output_dir "$output_dir" --fgwas_dir "$fgwas_dir" --ci_th "$ci_th" --ci_ppa_th "$ci_ppa_th"
    suff="gwas_CI.txt"
    ci_gwas_dir="${output_dir}${suff}"  
fi

if [ "$mode" == "ATAC_obj" ]; then
    Rscript EffectSNP.R --output_dir "$output_dir" --jaspar_mtx_dir "$jaspar_mtx_dir" --ci_gwas_dir "$ci_gwas_dir" --genome_build "$genome_build"
    Rscript Peak_cell_extract.R --seurat_obj_dir "$seurat_obj_dir" --output_dir "$output_dir" --peak_th "$peak_th" --cell_count_th "$cell_count_th"
    Rscript peak_interaction_extract.R --genome_build "$genome_build" --seurat_obj_dir "$seurat_obj_dir" --output_dir "$output_dir" --coaccess_th "$coaccess_th" --cic_genomic_window "$cic_genomic_window" --peak_th "$peak_th"
