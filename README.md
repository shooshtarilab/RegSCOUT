...
## Changes that have not been implemented as of July 20, 2025
In fgwas_data_prep.R, currently SNPs from the plink.afreq file are matched to GWAS summary statistics (line 126) by matching SNP IDs, which may vary depending on the version of dbSNP used. Jonathan suggested using SNP positions instead, this has not been implemented quite yet. 

Do not set the jaspar_mtx parameter to none, this will load in a matrix with slightly different structure than RegSCOUT expects and leads to an effect SNP output lacking TF names. This has not yet been fixed. 
